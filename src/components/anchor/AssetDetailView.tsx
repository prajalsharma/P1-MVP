...
export function AssetDetailView({ anchor }: AssetDetailViewProps) {
  ...
  const handleVerify = useCallback(
    async (file: File) => {
      ...
      setVerifyError(null);
      try {
        // Requirement: Re-hash MUST include the original UTC timestamp
        const { fingerprint } = await hashFile(file, anchor.created_at);
    ...
    [anchor.file_fingerprint, anchor.created_at]
  );
      ...
      <dl className={styles.meta}>
        ...
        <div className={styles.row}>
          ...
          <div className={styles.row}>
            ...
            <dd className={styles.val}>{formatTimestamp(anchor.created_at)}</dd>
          </div>
          {anchor.jurisdiction && (
            <div className={styles.row}>
          ...
          <div className={styles.chainSection}>
            ...
            <dl className={styles.chainMeta}>
              ...
              {anchor.chain_timestamp && (
                <div className={styles.row}>
                  <dt className={styles.key}>Anchored</dt>
                  <dd className={styles.val}>{formatTimestamp(anchor.chain_timestamp)}</dd>
                </div>
              )}
