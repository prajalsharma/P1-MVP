...
To ensure global uniqueness and satisfy European compliance for timestamped evidence, every anchor fingerprint MUST include the record's creation timestamp in the hash calculation.

1.  **Input**: File binary content + ISO UTC creation timestamp string.
2.  **Algorithm**: SHA-256 (Web Cryptography API).
3.  **Procedure**:
    - The creation timestamp is generated at the moment of first hashing.
    - The timestamp string is appended to the file bytes.
    - The combined buffer is hashed to produce the final `file_fingerprint`.

To re-verify a document, the system MUST use the same `created_at` timestamp from the database record as the input to the hash function.
